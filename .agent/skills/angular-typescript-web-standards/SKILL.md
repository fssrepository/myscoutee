...
- NEVER DELETE VALIDATED INTERACTION/MOTION BLOCKS (`:HOST` VARIABLES, HOVER/ACTIVE/TOUCH ANIMATION, FOCUS RINGS); APPLY INCREMENTAL PATCHES ONLY.
- NEVER DELETE OR REVERT ANY UI/CODE ELEMENT THE USER EXPLICITLY ASKED TO KEEP, UNLESS THE USER EXPLICITLY APPROVES THAT REVERT.
- WHEN USER ASKS FOR ORIGINAL/PARITY BEHAVIOR, PATCH CURRENT CODE IN PLACE; NEVER "RESET BY REBUILDING" THE SCREEN FROM SCRATCH.
- NEVER REMOVE, REVERT, OR MODIFY ANY UNREQUESTED FUNCTIONAL BLOCK; APPLY ONLY THE EXPLICITLY REQUESTED SCOPE.
- SCSS CHANGES ARE INCREMENTAL-ONLY BY DEFAULT: NEVER WHOLESALE-REWRITE/TRUNCATE/REPLACE COMPONENT STYLESHEETS.
- ANY LARGE-SCOPE SCSS REWRITE OR DELETION REQUIRES EXPLICIT USER APPROVAL FIRST.
...
- NEVER SIMPLIFY OR REMOVE BASELINE DATE-RANGE/CAPACITY/OVERLAP RULES WHEN MOVING SUBEVENT LOGIC OUT OF MONOLITH FILES.
- FOR SUBEVENT INSERT/EDIT FLOWS, KEEP BASELINE OVERLAP SEMANTICS EXACT: TRIM PARTIAL OVERLAP, SHIFT FULL/PARTIAL FOLLOWERS, AND RESYNC MAIN EVENT START/END/CAPACITY.

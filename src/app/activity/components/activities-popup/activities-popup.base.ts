...
import { ActivitiesRateEditorPresenter } from './templates/rate/activities-rate-editor.presenter';
import { ActivitiesPopupTemplatePresenter } from './templates/activities-popup-template.presenter';
import {
...
@Directive()
export abstract class ActivitiesPopupBase implements OnDestroy {
  ...
  protected readonly templatePresenter = new ActivitiesPopupTemplatePresenter({
    ...
    getChatLastSender: (chat) => this.getChatLastSender(chat),
    getChatMemberCount: (chat) => this.getChatMemberCount(chat),
    getChatChannelType: (chat) => this.chatChannelType(chat),
    ...
    isPendingActivityRow: (row) => this.isPendingActivityRow(row),
    isActivityFull: (row) => this.isActivityFull(row),
    getActivityLeadingIcon: (row) => this.activityLeadingIcon(row),
    getActivityLeadingIconTone: (row) => this.activityLeadingIconTone(row),
    ...
    getActivityEventInfoCardMenuActions: (row) => this.activityEventInfoCardMenuActions(row),
    getDisplayedRateDirection: (item) => this.displayedRateDirection(item),
    isSelectedActivityRateRow: (row) => this.isSelectedActivityRateRow(row),
    isActivityRateBlinking: (row) => this.isActivityRateBlinking(row),
    getActivityRateDraftValue: (itemId) => this.activityRateDraftById[itemId],
    normalizeRateScore: (value) => this.normalizeRateScore(value),
  ...
  private readonly rateEditorPresenter = new ActivitiesRateEditorPresenter({
    getActivitiesRateFilter: () => this.activitiesRateFilter,
    getSelectedRow: () => this.selectedActivityRateRow(),
    getFocusedRow: () => this.isRatesFullscreenModeActive()
      ? this.currentActivitiesRatesFullscreenRow()
      : this.selectedActivityRateRow(),
    isFullscreenModeActive: () => this.isRatesFullscreenModeActive(),
    isEditorClosing: () => this.isActivityRateEditorClosing(),
    isEditorOpen: () => this.isActivityRateEditorOpen(),
    getRatingScale: () => this.activityRatingScale,
    getOwnRatingValue: (row) => this.templatePresenter.activityOwnRatingValue(row),
    isPairReceivedRateRow: (row) => this.templatePresenter.isPairReceivedRateRow(row)
  });
  ...
  protected readonly activitiesSmartListConfig: SmartListConfig<AppTypes.ActivityListRow, ActivitiesSmartListFilters> = {
    ...
    pagination: {
      ...
      ratingBarConfig: () => this.rateEditorPresenter.barConfig(),
      ratingBarValue: () => this.rateEditorPresenter.selectedValue(),
  ...
  protected abstract clearActivityRateEditorState(preserveScrollPosition?: boolean): void;
  protected abstract openActivityRateEditor(row: AppTypes.ActivityListRow, event?: Event): void;
  protected abstract syncActivitiesRatesFullscreenSelection(): void;
  protected abstract currentActivitiesRatesFullscreenRow(): AppTypes.ActivityListRow | null;
  protected abstract isActivitiesRatesFullscreenReadOnlyNavigation(): boolean;
  protected abstract shouldRenderActivityRateEditorDock(): boolean;
  protected abstract isActivityRateEditorOpen(): boolean;
  protected abstract isActivityRateEditorClosing(): boolean;
  protected abstract isSelectedActivityRateRow(row: AppTypes.ActivityListRow): boolean;
  protected abstract isActivityRateBlinking(row: AppTypes.ActivityListRow): boolean;
  protected abstract activityRateEditorSpacerHeight(): string | null;
  protected abstract setSelectedActivityOwnRating(score: number): void;
  protected abstract displayedRateDirection(item: RateMenuItem): RateMenuItem['direction'];
  protected abstract selectedActivityRateRow(): AppTypes.ActivityListRow | null;
  protected abstract normalizeRateScore(value: number): number;
  protected abstract rateOwnScore(item: RateMenuItem): number;
  protected abstract hasOwnRating(item: RateMenuItem): boolean;
  protected abstract pairReceivedAverageScore(item: RateMenuItem): number;
  protected abstract matchesRateFilter(item: RateMenuItem, filter: AppTypes.RateFilterKey): boolean;
  protected abstract commitPendingRateDirectionOverrides(targetFilter?: AppTypes.RateFilterKey): void;
  protected abstract activityLeadingIcon(row: AppTypes.ActivityListRow): string;
  protected abstract activityLeadingIconTone(row: AppTypes.ActivityListRow): NonNullable<InfoCardData['leadingIcon']>['tone'];
  protected abstract isPendingActivityRow(row: AppTypes.ActivityListRow): boolean;
  protected abstract activityEventInfoCardMenuActions(row: AppTypes.ActivityListRow): readonly InfoCardMenuAction[];
  protected abstract getChatLastSender(item: ChatMenuItem): DemoUser;
  protected abstract getChatMemberCount(item: ChatMenuItem): number;
  protected abstract chatChannelType(item: ChatMenuItem): AppTypes.ChatChannelType;
  protected abstract chatItemsForActivities(): ChatMenuItem[];
  protected abstract activityChatContextFilterKey(item: ChatMenuItem): AppTypes.ActivitiesChatContextFilter | null;
  protected abstract openActivityChat(chat: ChatMenuItem): void;
  protected abstract onActivityRowClick(row: AppTypes.ActivityListRow, event?: Event): void;
  protected abstract openActivityMembers(row: AppTypes.ActivityListRow, event?: Event): void;
  protected abstract isActivityIdentityTrashed(type: AppTypes.ActivityListRow['type'], id: string): boolean;
  protected abstract trashedActivityCount(): number;
  protected abstract openActivityRowInEventModule(row: AppTypes.ActivityListRow, readOnly: boolean): void;

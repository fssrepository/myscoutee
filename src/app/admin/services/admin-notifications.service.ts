...
export class AdminNotificationsService {
  ...
  private readonly repository = inject(AdminNotificationsRepository);
    ...
    const nextRules = state.rules.map(rule => {
      ...
      return {
        ...
        runState: {
          ...
          lastRunCount: count,
          lastRunUser: this.workspace.activeAdmin()?.id ?? 'demo-admin'
        },
        runHistory: [{
          id: `run-${Date.now()}`,
          trigger: 'manual',
          runnerUser: this.workspace.activeAdmin()?.id ?? 'demo-admin',
          startedAtIso,
          finishedAtIso: nowIso,
          durationMillis: 1150,
        ...
        updatedDate: nowIso,
        updatedUser: this.workspace.activeAdmin()?.id ?? 'demo-admin'
      };
    });
  ...
  subscribeNotificationRuleUpdates(onEvent: (event: AdminNotificationRuleLiveEvent) => void): () => void {
    if (!this.workspace.usesHttpAdminApi || typeof WebSocket === 'undefined' || typeof window === 'undefined') {
      return () => {};
    }
    let closed = false;

...
export class UsersService extends BaseRouteModeService {
  ...
  peekCachedUsers(): UserDto[] {
    ...
    const persistedUsers = this.demoModeEnabled
      ? this.demoUsersRepository.queryAllUsers()
      : this.readHttpCachedUsers();
  ...
  peekCachedUserById(userId: string): UserDto | null {
    ...
    if (this.demoModeEnabled) {
      return this.demoUsersRepository.queryUserById(normalizedUserId);
    }
    const cachedResponse = this.offlineCache.readUser(normalizedUserId);
    return cachedResponse?.user ? this.cloneUser(cachedResponse.user) : null;
  }
  ...
  async warmCachedUsers(userIds: readonly string[]): Promise<void> {
    ...
    await Promise.all(pendingUserIds.map(async userId => {
      try {
        ...
        // Keep directory warmup best-effort so feature screens stay resilient.
      }
    }));
  }
  ...
  async loadUserById(userId?: string, requestTimeoutMs?: number): Promise<UserDto | null> {
    ...
    try {
      ...
      this.cacheResolvedUser(response.user);
      if (resolvedUserId) {
  ...
  async saveUserProfile(user: UserDto): Promise<UserDto | null> {
    ...
    try {
      ...
      this.cacheResolvedUser(resolvedUser);
  ...
  private readHttpCachedUsers(): UserDto[] {
    return this.offlineCache.readKnownUsers().map(user => this.cloneUser(user));
  }

  private cacheResolvedUser(user: UserDto): void {
    if (this.demoModeEnabled || !user?.id?.trim()) {
      return;
    }
    const normalizedUser = this.cloneUser(user);
    this.offlineCache.writeUser(normalizedUser.id, { user: normalizedUser });
  }

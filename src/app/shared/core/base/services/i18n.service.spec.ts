...
describe('I18nService', () => {
  ...
  it('uses Mongo-backed Hungarian install-prompt text after the static fallback', async () => {
  ...
  it('keeps the static install-prompt text as the HTTP fallback when Mongo is unavailable', async () => {
    get.mockImplementation((url: string) => {
      if (url === 'assets/i18n/en.json') {
        return of({
          lang: 'en',
    ...
    await vi.waitFor(() => {
      expect(service.translateParams(
        'add.myscoutee.to.your.home.screen',
        { productName: 'Fallback Brand' }
      )).toBe('Add static Fallback Brand to your home screen');
      expect(service.translateParams(
        'install.prompt.description',
        { productName: 'Fallback Brand' }
      )).toBe('Install static Fallback Brand from your home screen');
    });
  });

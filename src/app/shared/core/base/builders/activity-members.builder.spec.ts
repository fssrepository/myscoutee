...
describe('ActivityMembersBuilder', () => {
  it('orders pending actions first, then managers, then members with newest actions first', () => {
    const entries = [
    ...
    expect(ActivityMembersBuilder.sortActivityMembersForManagement(entries).map(entry => entry.userId))
      .toEqual([
        'pending-new',
        'pending-old',
        'manager',

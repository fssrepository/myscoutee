...
import { DemoActivityMembersRepository } from './activity-members.repository';
...
export class DemoEventFeedbackRepository {
  ...
  seedEventFeedbackStates(): void {
    ...
    for (const activeUser of users) {
      ...
      const seededRecords = DemoEventFeedbackBuilder.buildSeededPersistedStates({
        ...
        eventFeedbackUnlockDelayMs: DemoEventFeedbackRepository.EVENT_FEEDBACK_UNLOCK_DELAY_MS,
  ...
  private seedOrganizerFeedbackShowcaseRecords(
    ...
    for (const record of hostedEventById.values()) {
      ...
      if (visibleEntryCount >= DemoEventFeedbackRepository.ORGANIZER_FEEDBACK_SHOWCASE_TARGET_COUNT) {
        continue;
      }
  ...
  private organizerFeedbackShowcaseViewerUserIds(
    ...
    for (let index = 0; index < candidates.length && selected.length < DemoEventFeedbackRepository.ORGANIZER_FEEDBACK_SHOWCASE_TARGET_COUNT; index += 1) {

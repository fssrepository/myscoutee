...
import type { ContentModerationDecision, ContentModerationPage, ContentModerationSettings, ContentModerationSnapshot, ModerationCategory, ModerationStatus } from '../../contracts/content-moderation.interface';
...
export class HttpContentModerationService {
  ...
  page(adminUserId: string, category: ModerationCategory, status: ModerationStatus, query: ListQuery) {
  ...
  decide(id: string, request: ContentModerationDecision) { return firstValueFrom(this.http.post<ContentModerationSnapshot>(`${this.url}/${encodeURIComponent(id)}/decision`, request)); }

...
export class SeedBootstrapRegistryService {
  ...
  private assetsByUserId = new Map<string, readonly AssetRecord[]>();
  ...
  registerAssetsByUserId(assetsByUserId: ReadonlyMap<string, readonly AssetRecord[]>): void {
    this.assetsByUserId = new Map(assetsByUserId);
  }

  getAssetsByUserId(): ReadonlyMap<string, readonly AssetRecord[]> {

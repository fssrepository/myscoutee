...
export class LocalEventSeedBuilder {
  ...
  static seededEventMemberIds(
    ...
    if (!normalizedActiveUserId || LocalUserSeedBuilder.isEmptyOnboardingProfileUserId(normalizedActiveUserId)) {
      return [];
    }
    const seedableUsers = users.filter(user => !LocalUserSeedBuilder.isEmptyOnboardingProfileUserId(user.id));
  ...
  static buildSeededSubEventsForEvent(
    ...
    const startMs = Number.isNaN(start.getTime()) ? LocalSeedScheduleBuilder.anchorDate().getTime() : start.getTime();

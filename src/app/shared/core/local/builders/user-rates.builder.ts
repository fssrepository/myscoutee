...
export class LocalUserRatesBuilder {
  ...
  static buildGeneratedRateItemsForUser<TUser extends RateUserRef>(
    ...
    const extraSingleGivenCount = Math.max(
      0,
      Math.trunc(options.extraSingleGivenCount ?? LocalUserRatesBuilder.DEFAULT_EXTRA_SINGLE_GIVEN_COUNT)
    );
  ...
  private static buildGeneratedRateItemForLane(
    ...
    const happenedAtDate = LocalSeedScheduleBuilder.shiftDate(new Date('2026-03-01T20:00:00'));
  ...
  private static normalizeCoverageRatio(value: number | undefined): number {
    if (!Number.isFinite(value)) {
      return LocalUserRatesBuilder.DEFAULT_USER_COVERAGE_RATIO;
    }

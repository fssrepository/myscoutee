...
const DEMO_EVENT_MEMBER_USERS = LocalUserSeedBuilder.buildExpandedDemoUsers(50)
...
export class LocalEventsRepositoryBuilder {
  ...
  private static resolveLifecycleStatus(record: Pick<ActivityEventRecord, 'type' | 'isInvitation' | 'isTrashed' | 'published'>): ActivityEventRecord['status'] {
    if (record.isTrashed) {
      return 'T';
    }
  ...
  > {
    ...
    return {
      ...
      topics,
      subEvents,
      subEventsDisplayMode: record.seed?.subEventsDisplayMode ?? LocalEventSeedBuilder.inferredSubEventsDisplayMode(subEvents),
      rating,
  ...
  static resolveEventAffinity(options: {
    ...
    const participantAffinities = participantUsers
      ...
      .map(user => LocalUserSeedBuilder.resolveUserAffinity({
        id: `${user.id}`,
  ...
  private static resolveEndAtIso(
    ...
    if (Number.isNaN(startAt.getTime())) {
      return AppUtils.toIsoDateTimeLocal(LocalSeedScheduleBuilder.anchorDate());
    }
  ...
  private static resolveRecordLocationCoordinates(
    ...
    return LocalUserSeedBuilder.resolveDemoLocationCoordinates(
      creator.city || 'Austin',
  ...
  private static buildSeededMemberIds(
    ...
    const members = LocalEventSeedBuilder.seededEventMemberIds(
      record.id,
  ...
  private static buildSeededSubEvents(
    ...
    return LocalEventSeedBuilder.buildSeededSubEventsForEvent(source, {
  ...
  static rebaseSeedDateTime(value: string | Date | null | undefined): string | undefined {
    return LocalSeedScheduleBuilder.rebaseDateTime(value);
  }
  ...
  private static resolveLocationCoordinatesFromCreator(creator: UserDto): LocationCoordinates {
    return LocalUserSeedBuilder.resolveDemoLocationCoordinates(creator.city, creator.id);
  }

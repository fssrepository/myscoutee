...
import { LocalUserMenuCountersBuilder } from './user-menu-counters.builder';

interface LocalUsersRepositoryActivitySources {
...
export class LocalUsersRepositoryBuilder {
  ...
  static applySeededActivityCounts(
    user: UserDto,
    sources: LocalUsersRepositoryActivitySources
  ): UserDto {
    ...
    const chat = sources.chatItems
      ? LocalUserMenuCountersBuilder.resolveSectionBadge(sources.chatItems.map(item => item.unread), sources.chatItems.length)
      : activities.chat;
